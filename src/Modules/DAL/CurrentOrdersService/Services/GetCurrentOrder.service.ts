...
export class GetCurrentOrderService {
  ...
  async getCurrentOrders(mainOrderData: MainOrderDataSearch): Promise<CurrentOrderSchemaDocument[]> {
    ...
    try {
      ...
      console.log(e);
      throw new HttpException('Current orders not found.', HttpStatus.NOT_FOUND);
    }
  }
  ...
  private _getSearchObject(mainOrderData: MainOrderDataSearch) {
    ...
    searchObject = direction ? { ...searchObject, 'mainOrderData.direction': direction } : searchObject;
    searchObject = date ? { ...searchObject, 'mainOrderData.date': date } : searchObject;
    searchObject = startHour ? { ...searchObject, 'mainOrderData.startHour': startHour } : searchObject;
